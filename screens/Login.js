...
// Importation de la fonction `signInWithEmailAndPassword` depuis le module "firebase/auth"
import { signInWithEmailAndPassword } from "firebase/auth";
// Importation de l'objet `auth` depuis le module de configuration "firebase"
import { auth } from "../config/firebase";

// Importation d'une image depuis le fichier "Purple.jpg"
const Purple = require("../assets/Purple.jpg")

// Définition du composant fonctionnel `Login` avec une fonction de navigation en tant que paramètre
export default function Login({ navigation }) {

  // Déclaration de deux états pour stocker les valeurs des champs 'email' et 'password'
  ...
  // Définition de la fonction `onHandleLogin` qui gère le processus de connexion
  const onHandleLogin = () => {
    // Vérification si les champs 'email' et 'password' ne sont pas vides
    if (email !== "" && password !== "") {
      // Utilisation de la fonction `signInWithEmailAndPassword` pour tenter la connexion
  ...
  return (
    ...
    <View style={styles.container}>
      ...
      <SafeAreaView style={styles.form}>
        {/* Texte affichant "Connexion" avec le style défini dans la variable styles.title */}
        ...
        <TextInput
          ...
          placeholder="Saisir votre email"  // Texte indicatif affiché avant que l'utilisateur ne saisisse quelque chose
        ...
        <TextInput
          style={styles.input}  // Style du champ de saisie défini dans la variable styles.input
          placeholder="Saisir votre mot de passe"  // Texte indicatif affiché avant que l'utilisateur ne saisisse quelque chose
          autoCapitalize="none"  // Désactivation de la capitalisation automatique
        ...
        />
        {/* Bouton tactile avec le style défini dans la variable styles.button, et appel de la fonction onHandleLogin lorsqu'il est pressé */}
        ...
        <View style={{ marginTop: 20, flexDirection: 'row', alignItems: 'center', alignSelf: 'center' }}>
          {/* Texte indiquant la possibilité de s'inscrire */}
          <Text style={{ color: 'gray', fontWeight: '600', fontSize: 14 }}>Vous n'avez pas de compte ? </Text>

          {/* Bouton tactile qui, lorsqu'il est pressé, navigue vers l'écran d'inscription */}
